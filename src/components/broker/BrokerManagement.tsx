...
export function BrokerManagement({ onBack, onRegister, onClientLogin }: { onBack: () => void; onRegister: () => void; onClientLogin?: () => void }) {
  ...
  if (error) return <div className="empty-state"><p role="alert">{error}</p><button className="quiet-button" onClick={() => setAttempt(value => value + 1)}>إعادة المحاولة</button><button className="text-button" onClick={onBack}>العودة إلى المؤجر</button></div>;
  if (reviewer === null) return <p className="empty-state" role="status">جارٍ التحقق من صلاحية الحساب…</p>;
  return reviewer ? <BrokerReviewPanel onBack={onBack} onClientLogin={onClientLogin} /> : <><h3 className="review-title">طلب المؤجر الخاص بي</h3><BrokerRequestPanel onBack={onBack} onRegister={onRegister} onClientLogin={onClientLogin} /></>;
}
