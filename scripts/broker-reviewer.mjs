...
import { ensureBrokerReviewPermission } from '../server/accountPermissions.ts';
...
if (!['grant', 'revoke'].includes(action) || !email || extra.length || email.length > 254 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  console.error('Usage: node scripts/broker-reviewer.mjs <grant|revoke> <existing-account-email>');
  process.exitCode = 1;
} else {
  ...
  try {
    ...
    const user = db.prepare('UPDATE client_users SET can_review_brokers=? WHERE email=? RETURNING id').get(action === 'grant' ? 1 : 0, email);
    if (!user) throw new Error('No existing account matches this email. No permission was granted.');
    console.log(JSON.stringify({ userId: user.id, reviewBrokers: action === 'grant' }));
